...
export function loadTtsConfig(input: NodeJS.ProcessEnv): TtsConfig {
  const type = z.enum(['local', 'openai-compatible']).parse(input.TTS_PROVIDER_TYPE ?? 'local');
  const name = input.TTS_PROVIDER_NAME ?? (type === 'local' ? 'local' : 'openai-compatible');
  const apiKeyEnv = input.TTS_API_KEY_ENV ?? 'TTS_API_KEY';
  if (!/^[A-Z_][A-Z0-9_]*$/.test(apiKeyEnv)) throw new Error('TTS_API_KEY_ENV must be a valid environment variable name');
  ...
  const config: TtsConfig = {
    providerName: name, providerType: type, baseUrl: input.TTS_BASE_URL,
    apiKey: input[apiKeyEnv], apiKeyEnv, model: input.TTS_MODEL, voice: input.TTS_VOICE ?? 'Fritz-PlayAI',
  ...
  if (type !== 'local') {
    if (!config.baseUrl || !config.model || !config.apiKey) throw new Error(`Missing non-local TTS configuration: ${[!config.baseUrl && 'TTS_BASE_URL', !config.model && 'TTS_MODEL', !config.apiKey && apiKeyEnv].filter(Boolean).join(', ')}`);

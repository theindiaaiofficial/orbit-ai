...
describe('conversation memory and streaming', () => {
  it('uses persisted conversation history for a bounded follow-up', async () => {
    const first = await app.inject({ method: 'POST', url: '/v1/chat', headers: { 'x-api-key': key, origin }, payload: { message: 'What emergency service do you offer?' } });
    const second = await app.inject({ method: 'POST', url: '/v1/chat', headers: { 'x-api-key': key, origin }, payload: { message: 'Summarize what you just told me.', sessionId: first.json().sessionId } });
    expect(second.statusCode).toBe(200); expect(second.json().answer).toContain('emergency plumbing');
  });
  it('exposes progressive SSE events and a final persisted result', async () => {
    const r = await app.inject({ method: 'POST', url: '/v1/chat/stream', headers: { 'x-api-key': key, origin }, payload: { message: 'What service do you offer?' } });
    expect(r.statusCode).toBe(200); expect(r.headers['content-type']).toContain('text/event-stream'); expect(r.headers['access-control-allow-origin']).toBe(origin); expect(r.headers.vary).toContain('Origin'); expect(r.body).toContain('event: token'); expect(r.body).toContain('event: done');
  });
});
